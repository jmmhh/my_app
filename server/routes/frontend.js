...
router.get('/allArticle',(req,res)=>{
    ...
    db.Query(sql).then(data=>{
        for(let i=0;i<data.length;i++){
            data[i].imgsrc="http://127.0.0.1:8081"+data[i].imgsrc
        }
...
router.get('/oneArticle',(req,res)=>{
    ...
    console.log('获取单篇文章');
    db.Query(sql).then(data=>{
        data[0].imgsrc="http://127.0.0.1:8081"+data[0].imgsrc
        count = data[0].seecount+1;
        res.send({"code":"200","data":data[0]});
        let sqlStr=`update article set seecount=${count} where id=${id}`
