...
app.use(expressJWT({
    ...
    path: ['/api/user/login','/api/user/reguser' ,/^\/img\//]  //这里可以设置保护路由，login就不用进行token验证
}));
...
app.use((err,req,res,next)=>{
    if (err.name === 'ValidationError') return res.send({"code":"401",message:'用户名不合法'});
    if (err.name === 'UnauthorizedError') return res.send({"code":"401",message:'身份认证过期,请重新登陆!'});
    console.log(err.name);
